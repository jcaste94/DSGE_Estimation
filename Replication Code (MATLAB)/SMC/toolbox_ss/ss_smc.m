% ----------------------------------------------------------------
% ss_smc.m
% -Script for SMC algorithm in Herbst and Schorfheide (2014)
...
f = @(x1, x2) objfcn_smc(x1, x2); % x1: para, x2: tempering parameter
...
% ------------------------------------------------------------------------
estimMean = zeros(tune.nphi, tune.npara);
smctime   = tic;
totaltime = 0;
...
for i=2:1:tune.nphi
    ...
    if (ESS < tune.npart/2)
     ...
     %   [id] = multinomial_resampling(wtsim(:,i)'); %multinomial resampling
        
        id = randsample(length(loglh), length(loglh), true, wtsim(:,i)); 
    ...
    %%% This used to be parasim(i-1,:,:) - changed it to i
...
cd(savepath);
save(savefilename, 'parasim', 'wtsim', 'tune', 'csim', 'ESSsim', 'acptsim', 'rsmpsim');
cd(workpath);
